...
def require_admin_role(x_user_role: str | None = Header(default=None, alias="X-User-Role")) -> None:
    settings = get_settings()
    if not settings.enforce_rbac:
        return
...
def require_viewer_role(x_user_role: str | None = Header(default=None, alias="X-User-Role")) -> None:
    settings = get_settings()
    if not settings.enforce_rbac:
        return
